...
$code = $_GET['code'];
$verify = mysqli_query($con, "SELECT * FROM user WHERE code='$code' ");
  ...
  if ($validation->run()) {
    ...
    $cnfrmpass = $validation->input('cnfrmpass');
        ...
        if ($newpass == $cnfrmpass) {
            ...
            $update = mysqli_query($con, "UPDATE user SET password = '$newpass' WHERE code='$code' ");
            if ($update){
            $deleteCode = mysqli_query($con, "UPDATE user SET code = 0 WHERE code='$code' ");
